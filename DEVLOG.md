...
- 正文章节右键菜单新增"删除正文章节"：将对应已定稿草稿移入垃圾桶，可恢复，二次确认。
- 定稿改为以草稿自身的章节号为准，不再因参数与草稿轻微不一致而误报"内部状态异常"。
- 定稿物理正文文件名清理全部 Windows 非法字符（`\ / : * ? " < > |`），避免带特殊字符标题导致写入失败。
- 定稿各写入步骤（内容/状态）检查返回值，失败明确报错。
...
## 2026-08-13: 用户视角优化 — 批量诚实反馈、重写入口与定稿安全
*** End Patch

- 批量写稿/定稿改为：有勾选时只处理勾选章节，无勾选才处理全部；结果按完成/跳过/失败分别提示，不再静默"全部成功"。
- 章节蓝图右侧新增"重新生成"入口，任意章节都可发起写作（前置条件仍校验），方便重写不满意的一章。
- 生成草稿被一致性门禁阻断时，该记录自动归档，避免被误判为"已有可用草稿"。
- 合并修稿禁止覆盖已定稿正文，防止正文文件与数据库分叉。
- 定稿各写入步骤（内容/状态）检查返回值，失败即明确报错，不再伪成功。
...
## 2026-08-13: 统一蓝图 → 草稿 → 正文产物链
*** End Patch

- 草稿列表改为直接读取 `drafts` 表，不再依赖未删除的蓝图列表；蓝图进入垃圾桶后，关联草稿和正文仍可见。
- 草稿箱不再重复展示 `finalized` 草稿，正文入口成为定稿内容的唯一入口。
- 定稿同一章节时自动归档旧的 finalized 版本，保持单一正文真相。
- 生成草稿工作流执行时重新读取目标章节最新蓝图，避免使用过期 Tab 快照；草稿入库失败会真正报错，编辑器使用 Gate 修复后的最终正文。
- 定稿校验草稿 ID 与章节号一致，并在正文文件写入成功后才标记 finalized；物理正文写入失败不再显示定稿成功。
- 定稿与后处理使用当前蓝图标题，蓝图 notes 完成后刷新蓝图编辑器。
- 新增 `db:draft-list-all`，为草稿、正文、蓝图状态链提供稳定的章节记录来源。
...
## 2026-08-13: 草稿/蓝图多选归档与项目垃圾桶

- 草稿箱支持勾选多个非定稿草稿，批量归档前弹出二次确认。
- 草稿归档改为兼容当前 `luobi://draft/{id}` 数据库路径，不再依赖旧版文件名解析。
- 蓝图表新增 `deleted_at` 软删除字段，章节蓝图支持勾选后批量移入垃圾桶；旧数据库通过 schema v3 迁移。
- 项目侧栏新增垃圾桶，支持查看草稿/蓝图、恢复和彻底删除，彻底删除始终二次确认。
...
## 2026-08-13: 蓝图完成显示、批量写作入口与 Toast 材质修复

- 章节蓝图工作流完成后强制从数据库刷新，自动打开章节蓝图 Tab；仅在 Tab 没有未保存编辑时同步，避免旧缓存覆盖新生成内容。
- 解析 AI 蓝图标题时替换 `{{number}}`/`{{chapter}}` 占位符，避免任务标题把模板原样显示到界面。
- 工作流完成 Toast 的背景透明度与右上 Portal 菜单统一，修复右下角弹窗仍像实体卡片的问题。
- 章节蓝图页新增“批量写草稿”和“批量定稿”：串行复用现有单章工作流，跳过已有草稿/定稿，失败时停止后续章节，不并发污染上下文。
